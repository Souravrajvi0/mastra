...
If the agent produces messages faster than the Observer can process them, the `blockAfter` safety threshold forces buffered activation to use the maximum activation ratio. Synchronous observation is only used when the threshold is met and there are no buffered chunks to activate. Buffered activation still preserves a minimum remaining context (the smaller of \~1k tokens or the configured retention floor).
...
| `observation.blockAfter` | `1.2` | Safety threshold as a multiplier of `messageTokens`. At `1.2`, maximum buffered activation is forced at 36k tokens (1.2 × 30k). Does not directly trigger synchronous observation while buffered chunks exist. |
...
| `reflection.blockAfter` | `1.2` | Safety threshold for reflection, same activation logic as observation. |
