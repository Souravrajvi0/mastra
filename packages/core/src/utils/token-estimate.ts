/**
 * Lightweight token estimation for context budgeting and output truncation.
 *
 * Heuristic rules match the `tokenx` estimator introduced in #16326. This module
 * is implemented locally so published CommonJS chunks never static-import the
 * ESM-only `tokenx` package (see #22609).
 */
...
export function estimateTokenCount(text: string, options: TokenEstimateOptions = {}): number {
  if (!text) return 0;
